...
Confluence.Templates.CreateSpace.createSpaceForm = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<form action="', soy.$$escapeHtml("/wiki"), '/spaces/createspace.action" method="post" class="aui" id="create-space-form"><p>', soy.$$escapeHtml("Content in Confluence is organised into spaces. A space is where your pages live and where your blog is posted. You can create as many spaces as you like."), '</p><fieldset><div class="field-group"><label for="name">', soy.$$escapeHtml("Space name"), '<span class="aui-icon icon-required"></span></label><input class="medium-field" name="name" autocomplete="off" ', (opt_data.name) ? 'value="' + soy.$$escapeHtml(opt_data.name) + '"' : '', '/>');
  ...
  output.append('</div><div class="field-group"><label for="key">', soy.$$escapeHtml("Space key"), '<span class="aui-icon icon-required"></span></label><input class="medium-field" name="key" ', (opt_data.key) ? 'value="' + soy.$$escapeHtml(opt_data.key) + '"' : '', '/><a class="icon icon-info" href="#" id="space-key-info-link">', soy.$$escapeHtml("Help"), '</a>');
  if (opt_data.fieldErrors && opt_data.fieldErrors['key']) {
    var errorList36 = opt_data.fieldErrors['key'];
  ...
  Confluence.Templates.CreateSpace.spaceFormPermissions({confluenceFlavour: opt_data.confluenceFlavour, spacePermission: (opt_data.spacePermission) ? soy.$$escapeHtml(opt_data.spacePermission) : 'edit'}, output);
  output.append('<input type="hidden" name="atl_token" value="', soy.$$escapeHtml(opt_data.atlToken), '" />', (opt_data.buttons) ? '<div class="buttons-container"><div class="buttons"><input name="create" class="button" type="submit" value="' + soy.$$escapeHtml("Add") + '" /><a href="' + soy.$$escapeHtml("/wiki") + '/dashboard.action">' + soy.$$escapeHtml("Cancel") + '</a></div></div>' : '', '</form>');
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.CreateSpace.createPersonalSpaceForm = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<form action="', soy.$$escapeHtml("/wiki"), '/spaces/docreatepersonalspace.action" method="post" class="aui" id="create-personal-space-form"><p>', "Your personal space is your own private workspace within Confluence. You can optionally let other people view or contribute to it.", '</p>');
  Confluence.Templates.CreateSpace.spaceFormPermissions({confluenceFlavour: opt_data.confluenceFlavour, spacePermission: (opt_data.spacePermission) ? soy.$$escapeHtml(opt_data.spacePermission) : 'edit'}, output);
  output.append('<input type="hidden" name="atl_token" value="', soy.$$escapeHtml(opt_data.atlToken), '" />', (opt_data.buttons) ? '<div class="buttons-container"><div class="buttons"><input name="create" class="button" type="submit" value="' + soy.$$escapeHtml("Add") + '" /><a href="' + soy.$$escapeHtml("/wiki") + '/dashboard.action">' + soy.$$escapeHtml("Cancel") + '</a></div></div>' : '', '</form>');
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.CreateSpace.spaceFormPermissions = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<fieldset class="group" id="permissions-group"><legend><span>', soy.$$escapeHtml("User permissions"), '</span></legend>', (opt_data.confluenceFlavour == 'ALACARTE') ? '<div class="field-group"><em>' + soy.$$escapeHtml("Your new space will have the default space permissions as configured in Administration.") + '</em></div>' : '<div class="radio"><input class="radio" type="radio" id="permission-edit" name="spacePermission" value="edit"' + ((opt_data.spacePermission == 'edit') ? ' checked="checked"' : '') + ' /><label for="permission-edit">' + soy.$$escapeHtml("Others can add and edit content") + '</label></div><div class="radio"><input class="radio" type="radio" id="permission-view" name="spacePermission" value="view"' + ((opt_data.spacePermission == 'view') ? ' checked="checked"' : '') + ' /><label for="permission-view">' + soy.$$escapeHtml("Others can only comment") + '</label></div><div class="radio"><input class="radio" type="radio" id="permission-private" name="spacePermission" value="private"' + ((opt_data.spacePermission == 'private') ? ' checked="checked"' : '') + ' /><label for="permission-private">' + soy.$$escapeHtml("Others cannot view content") + '</label></div><div class="field-group"><div class="description">' + soy.$$escapeHtml("You can always add and edit content. You can change these permissions later.") + '</div></div>', '</fieldset>');
  return opt_sb ? '' : output.toString();
};

(function(e){var d=function(g){var f=g.siblings(".error");e.getJSON(AJS.Confluence.getContextPath()+"/ajax/spaceavailable.action",{key:g.val()},function(h){f.remove();if(!h.available&&h.message){g.parent().append('<div id="'+g.attr("name")+'-error" class="error">'+h.message+"</div>")}})},a=function(f){var g=f.find("input[name='key']").first();g.generateFrom(f.find("input[name='name']").first(),{uppercase:false,validationCallback:function(){d(g)}});g.change(function(){d(g)});new AJS.Tooltip({anchor:f.find("#space-key-info-link"),text:"Usually a unique, short version of a space&#8217;s name. It is used in links to content in this space.",width:150})},b=function(){var g=e(Confluence.Templates.CreateSpace.createSpaceForm({atlToken:e("#atlassian-token").prop("content"),confluenceFlavour:AJS.Meta.get("confluence-flavour")})),f=function(){g.submit()},h=new AJS.ConfluenceDialog({width:540,height:370,id:"create-space-dialog",closeOnOutsideClick:true,onSubmit:f});a(g);h.addHeader("Add a space");h.addPanel("Panel 1",g);h.addSubmit("Add",f);h.addCancel("Cancel",function(){h.hide()});h.gotoPage(0);h.gotoPanel(0);return h},c=null;Confluence.CreateSpace={showDialog:function(){if(c!==null){c.show();return}c=b();c.show()},bindToForm:a}})(AJS.$);
AJS.toInit(function(d){var c=null,b=function(){var f=d(Confluence.Templates.CreateSpace.createPersonalSpaceForm({atlToken:d("#atlassian-token").prop("content"),confluenceFlavour:AJS.Meta.get("confluence-flavour")})),e=function(){f.submit()},g=new AJS.ConfluenceDialog({width:540,height:300,id:"create-personal-space-dialog",closeOnOutsideClick:true,onSubmit:e});g.addHeader("Add your personal space");g.addPanel("Panel 1",f);g.addSubmit("Add",e);g.addCancel("Cancel",function(){g.hide()});g.gotoPage(0);g.gotoPanel(0);return g},a=function(){if(c==null){c=b()}c.show();d("#create-personal-space-dialog button.button-panel-submit-button").focus()};d("#create-personal-space-link").click(function(){a();return false})});
...
Confluence.Templates.KeyboardShortcutsDialog.keyboardShortcutHelpLink = function(opt_data, opt_sb) {
  ...
  Confluence.Templates.Dialog.customHelpLink({href: soy.$$escapeHtml("http://docs.atlassian.com/confluence/docs-42/Keyboard+Shortcuts"), text: soy.$$escapeHtml("View All Shortcuts")}, output);
  output.append('</div>');
  return opt_sb ? '' : output.toString();
};
