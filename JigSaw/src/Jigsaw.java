...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		Vector<JigsawNode> followNodes = new Vector<JigsawNode>();
		this.isCompleted = false;
		openList.add(currentJNode);
		...
		while(!openList.isEmpty())
		{
			...
			followNodes = this.findFollowJNodes(currentJNode);
			for (int i = 0; i < followNodes.size(); i++)
			{
				openList.add(followNodes.get(i));
			}
	...
	private void estimateValue(JigsawNode jNode) {		
		int manhattanDis = getManhattanDistance(jNode); //曼哈顿距离
		int wrongPosCount = getWrongNumPos(jNode); //位置放错节点的个数
		int s =  wrongAdjacent(jNode); // 后续节点不正确的数码个数
		
		int evalue =  5 * manhattanDis + 4 * s + (int)(1 * wrongPosCount);
		//4 6 1
		//5 4 1
		jNode.setEstimatedValue(evalue);
	}
	// 后续节点不正确的数码个数
	private int wrongAdjacent(JigsawNode jNode)
	{
		...
		for(int index =1 ; index<dimension*dimension; index++){
			...
			//if ((index + dimension) < dimension * dimension && jNode.getNodesState()[index] + dimension != jNode.getNodesState()[index + dimension])
			//	s++;
		}
		return s;
	...
	//返回曼哈顿距离
	private int getManhattanDistance(JigsawNode jNode)
	{
		int[][] array = changeIntoArray(jNode);	
		int manhattanDis = 0;
		...
		for (int i = 0; i < length; i++)
		{
			for (int j = 0; j < length; j++)
			{
				...
				if (value == 0)
					continue;
				manhattanDis += (Math.abs(i - (value) /length) + Math.abs(j - (value + 4) % length));
			}
		}
		return manhattanDis ;
	}
	//返回位置放错的数字个数
	private int getWrongNumPos(JigsawNode jNode)
	{
		int [] arr = jNode.getNodesState();
		int count = 0;
	...
	//将jNode状态中的一维数组改成二维数组
	private int[][] changeIntoArray (JigsawNode jNode)
	{
		int d = JigsawNode.getDimension();
